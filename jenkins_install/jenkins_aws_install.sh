...
echo ""
echo "  Next steps (run remotely, over SSH with ${JENKINS_EC2_KEY}):"
echo "  1. Set the GitHub Webhook"
echo "     ssh -i ${JENKINS_EC2_KEY} ec2-user@${JENKINS_EC2_IP} '${REMOTE_HOME}/jenkins_install/setup_github_webhook.sh'"
echo "  2. Load the desired configuration and enable its pipeline(s)"
echo "     ssh -i ${JENKINS_EC2_KEY} ec2-user@${JENKINS_EC2_IP} '${REMOTE_HOME}/jenkins_configs/reload_jenkins_config.sh <config-name>.yaml'"
echo "=================================================="
echo ""
